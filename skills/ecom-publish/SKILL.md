...
3. 用户要求正式刊登文件但缺少会改变字段映射的信息时，一次集中询问站点模板、类目映射或自定义字段。
4. 正式刊登需要图片 URL 时，优先使用用户提供的永久 CDN URL。只有本地图片且用户只是内部审阅时允许导出本地路径；正式发布前明确标记其不可用并询问永久 URL。
5. 不重复询问 `listing.json`、`strategy.json` 或用户消息中已有的信息。
